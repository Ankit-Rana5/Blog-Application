...
public class SecurityConfig {
	 ...
	 * .authorizeHttpRequests(auth -> { try { auth.requestMatchers("/test")
	 * .authenticated() .requestMatchers("/api/v1/auth/login").permitAll()
	 * .anyRequest() .authenticated() .exceptionHandling(ex -> ex
	 * .authenticationEntryPoint(jwtAuthenticationEntryPoint))
	 * .sessionManagement(session -> session
	 * .sessionCreationPolicy(SessionCreationPolicy.STATELESS)); } catch (Exception
	 * e) {
	 * 
	 * // e.printStackTrace(); System.out.println("envalid password and user"); }
	 * });
	 * 
